...
def month_solved_count(db, date: datetime.date) -> int:
    return db.execute(
...
def _select_date(db, *args) -> datetime.date | None:
    res = db.execute(*args)
...
def last_unsolved(db, min_date=None):
    if min_date:
        return _select_date(
            db,
    ...
    return _select_date(
        db, "SELECT date FROM solved WHERE solved = ? ORDER BY date DESC", (False,)
    )
